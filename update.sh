...
if [ $CURRENT_VERSION != $AVAILABLE_VERSION ]; then
  ...
  #$wget -nv -O osTicket.zip https://github.com/osTicket/osTicket/releases/download/$AVAILABLE_VERSION/osTicket-$AVAILABLE_VERSION.zip
  $wget -q --show-progress https://github.com/osTicket/osTicket/releases/download/$AVAILABLE_VERSION/osTicket-$AVAILABLE_VERSION.zip
  mv -f osTicket-$AVAILABLE_VERSION.zip osTicket-latest.zip
  rm -r build
  mkdir build
  unzip osTicket-$AVAILABLE_VERSION.zip "upload/*" -d "build/"
  rm osTicket-$AVAILABLE_VERSION.zip
